...
public class AppDatabase {
	 ...
	 * namespace ID.  This method is required for updating data when upgrading to Application 0.9.0,
	...
	public static void migrateDatabase(Properties props, List<WikiMessage> errors) throws DataAccessException {
		...
		try {
			...
			// used to track current_version_id for each app_topic row inserted
			Map<Integer, Integer> topicVersions = new HashMap<Integer, Integer>();
			for (int i = 0; i < APPLICATION_DB_TABLE_INFO.length; i++) {
				// these 3 variables are for special handling of the app_topic.current_version_id field
				// which cannot be loaded on initial insert due to the app_f_topic_topicv constraint
				boolean isTopicTable = "app_topic".equals(APPLICATION_DB_TABLE_INFO[i][0]);
				int topicIdColumn = 0;
				int currentVersionColumn = 0;
				int maxIndex = WikiDatabase.retrieveMaximumTableId(APPLICATION_DB_TABLE_INFO[i][0], APPLICATION_DB_TABLE_INFO[i][1]);
				...
				for (int j = 0; j <= maxIndex; j += RECORDS_PER_CYCLE) {
					select = new StringBuilder("SELECT * FROM ").append(APPLICATION_DB_TABLE_INFO[i][0]);
					if (!StringUtils.isBlank(APPLICATION_DB_TABLE_INFO[i][1])) {
						if (j == 0) {
							// for the first record do not set a lower limit in case there is an ID less
							// than zero
							select.append(" WHERE ");
						} else {
							select.append(" WHERE ").append(APPLICATION_DB_TABLE_INFO[i][1]).append(" > ").append(j);
							select.append(" AND ");
						}
						select.append(APPLICATION_DB_TABLE_INFO[i][1]).append(" <= ").append(j + RECORDS_PER_CYCLE);
						select.append(" ORDER BY ").append(APPLICATION_DB_TABLE_INFO[i][1]);
					}
					...
					md = rs.getMetaData();
					insert.append("INSERT INTO ").append(APPLICATION_DB_TABLE_INFO[i][0]).append('(');
					...
					for (int k = 1; k <= md.getColumnCount(); k++) {
						...
						if (newQueryHandler instanceof com.rkc.zds.web.spring.wiki.db.SybaseASAQueryHandler && "login".equalsIgnoreCase(columnName)) {
			...
			// update the app_topic.current_version_id field that we had to leave blank on initial insert
			String updateSql = "UPDATE app_topic SET current_version_id = ? WHERE topic_id = ?";
	...
	public synchronized static void initialize() {
		try {
			AppDatabase.CONNECTION_VALIDATION_QUERY = JavaServerApp.getDataHandler().queryHandler().connectionValidationQuery();
	...
	private static Connection initializeNewDatabase(Properties props, List<WikiMessage> errors, QueryHandler newQueryHandler) {
		...
		// test to see if Application tables already exist (if they do, we can't continue this migration process
		Statement stmt = null;
		try {
			...
			// we expect this exception as the Application tables don't exist
			logger.debug("NEW Database does not contain any Application instance");
		} finally {
			DatabaseConnection.closeStatement(stmt);
		}
		try {
	...
	protected static void purgeData(Connection conn) throws DataAccessException {
		...
		JavaServerApp.getDataHandler().queryHandler().dropTables(conn);
		try {
			// re-create empty tables
			JavaServerApp.getDataHandler().queryHandler().createTables(conn);
		} catch (Exception e) {
	 ...
	 * /
	protected static String readSystemPage(Locale locale, String pageName) throws IOException {
	...
	protected static void releaseConnection(Connection conn, Object transactionObject) throws SQLException {
		...
		AppDatabase.releaseConnection(conn);
	}
	...
	protected static void setup(Locale locale, WikiUser user, String username, String encryptedPassword) throws DataAccessException, WikiException {
		...
		try {
			...
			WikiDatabase.setupSystemPages(locale, user);
		} catch (SQLException e) {
	...
	public static void setupDefaultDatabase(Properties props) {
		...
		String url = "jdbc:hsqldb:file:" + new File(file.getPath(), "application").getPath() + ";shutdown=true";
	...
	protected static void setupDefaultInterwikis() throws DataAccessException, WikiException {
		...
		Interwiki application = new Interwiki("applicationorg", "http://zdslogic.com/wiki/en/{0}", "Application");
		WikiBase.getDataHandler().writeInterwiki(application);
	 ...
	 *
	 * /
	protected static void setupSystemPage(Locale locale, String virtualWiki, String topicName, WikiUser user, boolean adminOnly, boolean readOnly) throws DataAccessException, WikiException {
		logger.info("Setting up special page " + virtualWiki + " / " + topicName);
		if (user == null) {
			throw new IllegalArgumentException("Cannot pass null WikiUser object to setupSystemPage");
		}
		String contents = null;
		try {
			contents = WikiDatabase.readSystemPage(locale, topicName);
		} catch (IOException e) {
			throw new DataAccessException(e);
		}
	 ...
	 *
	 * /
	private static void setupSystemPages(Locale locale, WikiUser user) throws DataAccessException, WikiException {
		List<VirtualWiki> all = WikiBase.getDataHandler().getVirtualWikiList();
		for (VirtualWiki virtualWiki : all) {
			// create the default topics
			setupSystemPage(locale, virtualWiki.getName(), WikiBase.SPECIAL_PAGE_STARTING_POINTS, user, false, false);
			setupSystemPage(locale, virtualWiki.getName(), WikiBase.SPECIAL_PAGE_SIDEBAR, user, true, false);
			setupSystemPage(locale, virtualWiki.getName(), WikiBase.SPECIAL_PAGE_FOOTER, user, true, false);
			setupSystemPage(locale, virtualWiki.getName(), WikiBase.SPECIAL_PAGE_HEADER, user, true, false);
			setupSystemPage(locale, virtualWiki.getName(), WikiBase.SPECIAL_PAGE_SYSTEM_CSS, user, true, true);
			setupSystemPage(locale, virtualWiki.getName(), WikiBase.SPECIAL_PAGE_CUSTOM_CSS, user, true, false);
		}
	}
